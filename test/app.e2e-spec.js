...
describe('Bank API (e2e)', () => {
    ...
    const account = {
        accountId: 'acc-1',
        personId: 'person-1',
        balance: new client_1.Prisma.Decimal(1000),
        dailyWithdrawalLimit: new client_1.Prisma.Decimal(500),
        activeFlag: true,
    ...
    const prismaMock = {
        ...
        $transaction: jest.fn((cb) => cb({
            ...
            transaction: {
                aggregate: jest
                    .fn()
                    .mockResolvedValue({ _sum: { value: new client_1.Prisma.Decimal(0) } }),
                create: jest.fn().mockImplementation(({ data }) => {
                    if (data.idempotencyKey && ledgerByKey.has(data.idempotencyKey)) {
                        return Promise.reject(new client_1.Prisma.PrismaClientKnownRequestError('dup', {
                            code: 'P2002',
                            clientVersion: 'test',
                        }));
                    }
                    ...
                    if (data.idempotencyKey)
                        ledgerByKey.set(data.idempotencyKey, txn);
        ...
        transaction: {
            findUnique: jest
                .fn()
                .mockImplementation(({ where }) => Promise.resolve(ledgerByKey.get(where.idempotencyKey) ?? null)),
        },
    };
    beforeAll(async () => {
    ...
    it('rejects a statement query where endDate precedes startDate (400)', () => {
        return (0, supertest_1.default)(app.getHttpServer())
            .get('/transactions/account/acc-1/range?startDate=2026-12-31&endDate=2026-01-01')
            .expect(400);
    });
